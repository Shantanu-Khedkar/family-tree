...
In the image above, the node 'Lauren' does not have any child nodes, hence the caret-down button is not visible. Also note how the search finds nodes (works for both spouse and member search)
...
## Contributions
As this is a very old personal project, the code is not exactly suitable to work on further... I have done my best to clean it up and get it working again, but that's all, so I don't expect futher progress on completing the TODO list in **index.html**. If I had to do work on it I would rather scrap this and make some React Native app that decrypts an XML file generated by a good tree creation front-end tool. Anyways, it is still important to recognise contributions made by my friend (@decypher) in getting the crytography and API processes working 
